...
function EquipesPage() {
  ...
  const load = useCallback(async () => {
    if (!lab) return;
    const { data } = await supabase.from("equipes").select("id, nom").eq("laboratoire_id", lab.id).order("nom");
  ...
  return (
    <div className="space-y-4 max-w-5xl">
      ...
      <Card>
        <Table>
          <TableHeader>
            <TableRow><TableHead>Nom</TableHead><TableHead className="w-32">Actions</TableHead></TableRow>
          </TableHeader>
          <TableBody>
            {rows.length === 0 ? (
              <TableRow><TableCell colSpan={2} className="text-center text-muted-foreground py-8">Aucune donnée disponible</TableCell></TableRow>
            ) : rows.map((r) => (
              <TableRow key={r.id}>
